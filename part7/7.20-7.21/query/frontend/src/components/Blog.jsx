  ...
  return (
    <Box >
      <Typography variant="h2">{blog.title} {blog.author}</Typography>
        ...
        <Typography variant="body1">{blog.likes} likes<Button variant="outlined" onClick={() => handleLike(blog)}>like</Button></Typography>
        <Typography variant="body1">added by {blog.user.name}</Typography>
        <Typography variant="h3">comments</Typography>
        <form onSubmit={handleComment}>
          <TextField  name="comment"/><Button variant='outlined' type="submit">add comment</Button>
        </form>

...
export const createBlog = newBlog => {
  return async dispatch => {
    try {
      ...
      dispatch(appendBlog({ ...responseBlog, user: { name: blogUser.name, username: blogUser.username } }))
...
export const commentBlog = ({ id, comment }) => {
  return async dispatch => {
    try{
      ...
      dispatch(setNotification(`you commented on '${responseBlog.title}'`, 'success', 5))
    } catch {
